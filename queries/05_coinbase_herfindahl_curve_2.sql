...
`project_name.bitcoin.herf_dates` AS
  SELECT
...
`project_name.bitcoin.herf_coinbase_output` AS
 ...
 CROSS JOIN
   `project_name.bitcoin.herf_dates` AS dates
 WHERE
   dates.date >= DATE(tx.block_timestamp)
   AND DATE_DIFF(dates.date, DATE(tx.block_timestamp), DAY) BETWEEN 0 AND 28 * 6
   AND tx.is_coinbase
   AND outputs.value > 0
;
...
CREATE TABLE IF NOT EXISTS
--CREATE OR REPLACE TABLE
`project_name.bitcoin.herf_tx` AS
 SELECT
...
`project_name.bitcoin.herf_coinbase_txo_flow` AS
 ...
 LEFT JOIN
   `project_name.bitcoin.herf_tx` AS tx1
 ON
 ...
 LEFT JOIN
   `project_name.bitcoin.herf_tx` AS tx2
 ON
 ...
 LEFT JOIN
   `project_name.bitcoin.herf_tx` AS tx3
 ON
 ...
 LEFT JOIN
   `project_name.bitcoin.herf_tx` AS tx4
 ON
...
CREATE TABLE IF NOT EXISTS
--CREATE OR REPLACE TABLE
`project_name.bitcoin.herf_txos` AS
SELECT
  days_since_coinbase,
  coinbase_tx_hash,
...
CREATE TABLE IF NOT EXISTS
--CREATE OR REPLACE TABLE
`project_name.bitcoin.herf_herf` AS
SELECT
...
FROM
  `project_name.bitcoin.herf_txos` AS txos
JOIN
(SELECT
  days_since_coinbase,
  coinbase_tx_hash,
...
FROM
  `project_name.bitcoin.herf_txos`
GROUP BY
...
FROM
  `project_name.bitcoin.herf_herf`
GROUP BY
  coinbase_tx_hash,
  coinbase_block_number,
  coinbase_block_ts,
